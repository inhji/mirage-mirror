defmodule Mirage.Indie.Webmentions do
  require Logger
  use Tesla

  alias Mirage.Indie.Webmentions.Utils
  alias Mirage.Indie.Webmentions.Response

  plug(Tesla.Middleware.FollowRedirects, max_redirects: 3)
  plug(Tesla.Middleware.FormUrlencoded)

  @spec send_webmentions(String.t(), String.t()) ::
  ...
  defp handle_send_webmention(source, target) do
    Logger.info("handle_send_webmention")
    ...
    with {:ok, response} <- get(source_url),
      ...
      cond do
        link != nil and is_webmention_link(link) ->
          Logger.info("Found webmention Link header")
          ...
          Logger.info("Extracted value: #{link}")
          ...
          Logger.info("Final value: #{abs_link}")

          {:ok, abs_link}

        is_text ->
          Logger.info("No header found, checking <head>")
  ...
  def results_as_html(list) do
    lines =
      Enum.map(list, fn
        %Response{status: :error, message: reason, target: dest, endpoint: endpoint}
        when not is_nil(endpoint) ->
          "<li><strong>ERROR:</strong> #{dest}: endpoint #{endpoint}: #{reason}</li>"

        %Response{status: status, message: reason, target: dest}
        when status in [:error, :no_endpoint] ->
